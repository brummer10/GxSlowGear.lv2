...
#define GTK_SLOW_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_SLOW_KNOB, GtkSlowKnobPrivate))

G_DEFINE_TYPE(GtkSlowKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkSlowKnobPrivate GtkSlowKnobPrivate;

struct _GtkSlowKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void print_value(GObject *widget, char* s)
{
    ...
    else if (gtk_adjustment_get_step_increment(GTK_ADJUSTMENT(widget)) < 0.009999)
	{
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkSlowKnobPrivate *priv = GTK_SLOW_KNOB_GET_PRIVATE(widget);
...
{
	g_assert(GTK_IS_SLOW_KNOB(widget));
	GtkSlowKnob *knob = GTK_SLOW_KNOB(widget);
	GtkSlowKnobPrivate *priv = GTK_SLOW_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkSlowKnobClass
 */

static void gtk_knob_class_init (GtkSlowKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkSlowKnobPrivate));
}
...
static void gtk_knob_init (GtkSlowKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_SLOW_KNOB, NULL ));
