...
G_DEFINE_TYPE(GxSlowPaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxSlowPaintBoxClass *klass)
{
...
static void set_paint_func(GxSlowPaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxSlowPaintBox *paint_box = GX_SLOW_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxSlowPaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxSlowPaintBox *paint_box = GX_SLOW_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_SLOW_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxSlowPaintBox *paintbox = GX_SLOW_PAINT_BOX(wi);
